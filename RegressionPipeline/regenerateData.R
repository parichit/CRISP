...
regenerate_data <- function(train_data, test_data, real_comp_best_models, imag_comp_best_models, 
    ...
    get_new_data <- function(){
      ...
      train_new_data <- arrange(train_new_data$Volt, train_new_data$Freq)
    ...
    real_comp_best_models = c("Rborist", "knn", "ranger")
    imag_comp_best_models = c("kknn", "ranger", "Rborist")
    number = 5
    repeats = 5
    set_seed = 142
    ...
    # imag_data <- read.csv2(file="/Users/schmuck/Library/CloudStorage/OneDrive-IndianaUniversity/PhD/DSAA_23/data/LowFreqData/synthetic_imag_data.csv", 
    #                        sep = ",", stringsAsFactors = FALSE)
    # 
    # imag_data = arrange(imag_data, Volt, Freq)
    ...
    for (i in v){
      temp <-  regenerated_data[regenerated_data$Volt == i, ]
      temp <- temp[order(temp$Freq, decreasing = TRUE), ]
      
      print(paste("Volt: ", i))
      print(paste("Total size: ", nrow(temp), "less than 1: ", nrow(temp[temp$Freq < 1,])))
      
      # print(paste("Volt: ", i, "Min:", min(temp$Freq), "Max: ", max(temp$Freq)))
    }
    
    
    
    write.table(regenerated_data, file=paste("Regenerated_kknn", file_name_string, "_data.csv", sep=""), row.names = FALSE, sep=",")
    # 
