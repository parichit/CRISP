...
out_dir = file.path(base_path, "lowFreqResults")


Input_data_folder = file.path("data", "LowFreqData")
Input_file_name = "EIS_ExtraLowFreq_Data.xlsx"
...
availableModels = install_missing_packages(file.path("regressionModelList.txt"))
...
source("lowFreqDataIO.R")

out <- load_data(file.path(base_path, Input_data_folder, Input_file_name))
train_data <- out[[1]]
test_data <- out[[2]]


source("runLowFreqModels.R")
